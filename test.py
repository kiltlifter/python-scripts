...
fqdn = "my.c2s2.local"
domain_components = "dc=my,dc=c2s2,dc=local"
dom_dot = fqdn.split(".")[0]
amadmin_hash = 'password'
...
with open("openam-config-data.ldif", "w") as openam_setup:
	openam_setup.write(base_entries)
	
with open("openam-base-entries.ldif", "w") as openam_defaults:
	openam_defaults.write(user_store)
