...
TEMPFILE='/tmp/ldap_temp.ldif'

echo -n 'Domain (dc=example,dc=com): '
read DOM
echo -n 'Domain (example.com): '
read DOM_DOT
DC=`echo $DOM_DOT | awk -F '.' '{print $1}'`
echo -n 'Password for amadmin: '
read -s PASS
echo ""
PASS=`slappasswd -s $PASS`

# add the openam schema to ldap
echo "dn: cn=openam,cn=schema,cn=config
...
dn: ou=people,dc=opensso,$DOM
objectClass: top
ou:people
objectClass: organizationalUnit

dn: ou=groups,dc=opensso,$DOM
ou:groups
objectClass: top
objectClass: organizationalUnit

dn: cn=amadmin,ou=people,dc=opensso,$DOM
...
dn:cn=defaultGroup,ou=groups,dc=opensso,$DOM
objectclass: top
objectclass: groupofnames
member:cn=amadmin,ou=people,dc=opensso,$DOM
cn:default1" > $TEMPFILE

echo "Connecting to ldap as Manager..."
ldapadd -x -D "cn=Manager,$DOM" -W -f $TEMPFILE 
