...
class TestAlfresco(unittest.TestCase):
    ...
    def test_alfresco(self, hostname="my.c2s2.local", document="/home/sdouglas/documents/C2S2-SAE-1.0.0.1.2-Build-Plan_additional_artifacts.xls"):
        ...
        time.sleep(1)
        driver.find_element_by_id("dialog:dialog-body:file-input").send_keys(document)
        # Breaks with firefox
        driver.find_element_by_id("dialog:finish-button").click()
        driver.find_element_by_id("dialog:finish-button").click()
        # Uncomment in production
        document_name = document.split("/")[-1]
        driver.find_element_by_link_text(document_name).click()
        ...
        os.popen("gnome-open " + document)
        # Go to the alfresco share portal
        driver.get("http://"+hostname+"/share/")
        time.sleep(1)
        ...
        for i in range(len(document_split)):
            if i == 0:
                document_pdf = document_split[i]
            elif i == (len(document_split) - 1):
                document_pdf += ".pdf"
            else:
                document_pdf = document_pdf + "." + str(document_split[i])

        driver.find_element_by_link_text(document_pdf).click()
        time.sleep(8)
        driver.find_element_by_xpath("//a[@title='Delete Document']").click()
        driver.find_element_by_xpath("//span[@class='button-group']/span[1]/span/button").click()
